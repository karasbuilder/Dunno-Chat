...
import ChatApp.android.Model.NotificationModel;

public class PushNotificationSendService extends AsyncTask<NotificationModel, Void, Void> {
    ...
    protected Void doInBackground(NotificationModel... nmodel) {
        String desToken = nmodel[0].receiver_token;
        ...
        try {
            json.put("to", desToken); //Desitnation token
            json.put("color", "#FF33CC"); //Icon color
            JSONObject info = new JSONObject();
            info.put("title", title); // Notification title
            info.put("body", body); // Notification
            // body
            json.put("notification", info); // Push type
